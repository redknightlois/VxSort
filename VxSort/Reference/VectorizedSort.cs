...
#pragma warning restore 649
            ...
            internal void HybridSort(int* left, int* right, long realignHint, int depthLimit)
            {
                ...
                if (length < SMALL_SORT_THRESHOLD_ELEMENTS)
                {
                    ...
                    {
                        //Console.WriteLine($"B:[{left - _startPtr}|{left + length - _startPtr}]");
                        BitonicSort.Sort(fakeLeft, nextLength);
                    }
                    else
                    {
                        //Console.WriteLine($"B:[{left - _startPtr}|{right - _startPtr}]");
                ...
                //var values = new Span<int>(_startPtr, (int)(_endPtr -_startPtr));
                //Console.WriteLine($"Start({depthLimit})=[{string.Join(',', values.ToArray())}]");
                ...
                //Console.WriteLine($"V:[{(left - _startPtr)}|{ (right - _startPtr)}]");
                ...
                //Console.WriteLine($"Sep:[{(sep - _startPtr)}|{*sep}]");

                //Console.WriteLine($"L:[{(left - _startPtr)}|{ (sep - 1 - _startPtr)}]");
                HybridSort(left, sep - 1, realignHint | REALIGN_RIGHT, depthLimit);
                //Console.WriteLine($"R:[{(sep - _startPtr)}|{ (right - _startPtr)}]");
                HybridSort(sep + 1, right, realignHint | REALIGN_LEFT, depthLimit);
                _depth--;

                //Console.WriteLine($"End({depthLimit})=[{string.Join(',', values.ToArray())}]");
            }
